...
public class DrawingView extends View {
	...
	static final int MODE_CENTRER = 4; // to center the view
	static final int MODE_DELETE = 5; // to center the view
	...
	MyButton centrerButton = new MyButton( "Encadrer", 10, 220, 140, 140 );
	MyButton deleteButton = new MyButton( "Effacer", 10, 370, 140, 140 );
	...
	protected void onDraw(Canvas canvas) {
		...
		centrerButton.draw( gw, currentMode == MODE_CENTRER );		
				...
				public boolean onTouch(View v, MotionEvent event) {
					...
					case MODE_NEUTRAL :
						if ( cursorContainer.getNumCursors() == 1 && type == MotionEvent.ACTION_DOWN ) {
							...
							}else if ( centrerButton.contains(p_pixels) ) {
								currentMode = MODE_CENTRER;
								cursor.setType( MyCursor.TYPE_BUTTON );
							}
					...
					case MODE_DELETE:
						if ( type == MotionEvent.ACTION_DOWN ) {
							if ( cursorContainer.getNumCursorsOfGivenType(MyCursor.TYPE_DRAGGING) == 0 ) {
								Point2D p_pixels = new Point2D(x,y);
								Point2D p_world = gw.convertPixelsToWorldSpaceUnits( p_pixels );
								int shapeIndex = shapeContainer.indexOfShapeContainingGivenPoint( p_world );
								if(shapeIndex >= 0) {
									Shape shapeDeleted = shapeContainer.getShape(shapeIndex);
									selectedShapes.remove(shapeDeleted);
									shapeContainer.shapes.remove(shapeDeleted);
								}
							}
							cursor.setType(MyCursor.TYPE_DRAGGING);							
						}
